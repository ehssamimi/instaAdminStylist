...
export default function ClaimDetailsPage() {
  ...
  const handleSubmitNote = async (files: File[]) => {
    console.log(`${modalType} claim with notes:`, notes);
          ...
          <div className='px-4'>
            ...
            <div className='flex mb-4 gap-4 border-b border-gray-100 pb-4'>
              {claimData?.data?.device_info?.receipt_photos?.[0] && (
                <div onClick={() => openImageModal(claimData.data.device_info.receipt_photos[0])} className="cursor-pointer hover:opacity-90 transition-opacity">
                  <Image src={claimData?.data?.device_info?.receipt_photos?.[0]} className='rounded' alt="iPhone 14 Pro" width={72} height={72} />
                </div>
              )}
              <div>
            ...
            <div className="flex justify-between items-center">
              <div className='flex mb-4 gap-4'>
                {claimData?.data?.device_info?.receipt_photos?.[0] && (
                  <div onClick={() => openImageModal(claimData.data.device_info.receipt_photos[0])} className="cursor-pointer hover:opacity-90 transition-opacity">
                    <Image src={claimData?.data?.device_info?.receipt_photos?.[0]} className='rounded' alt="iPhone 14 Pro" width={48} height={48} />
                  </div>
                )}
                <div>
