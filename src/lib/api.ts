...
import {
  WarrantyPriceFormData,
  WarrantyPricingItem,
  WarrantyPricingResponse,
} from "@/models/warrantyPricing";
...
import {
  ...
  ClaimsResponse,
  FileClaimPayload,
} from "@/models/claimOptions";
import {
  CoverageProductDetails,
  CoverageProductStartResponse,
} from "@/models/coverageProductStart";
import {
  CoverageProductsResponse,
  CoverageProductDetailResponse,
} from "@/models/coverageProducts";
...
import { normalizeAdminReviewsListResponse } from "@/lib/admin-reviews-normalize";
import type {
...
export const authApi = {
  ...
  verifyPasswordResetOtp: async (data: { email: string; otp: string }) => {
    return api.post<VerifyPasswordResetOtpResponse>(
  ...
  forgotPassword: async (email: string) => {
    return api.post<ForgotPasswordResponse>("/admin/auth/forgot-password", {
      email,
    });
  },

  resetPassword: async (data: ResetPasswordRequest) => {
    return api.post<ResetPasswordResponse>("/admin/auth/reset-password", data);
  },

  verifyPassword: async (password: string, resetTokenFromLink: string) => {
    const t = resetTokenFromLink.trim();
    return api.post<ResetPasswordResponse>("/admin/auth/reset-password", {
      token: t,
      password,
    });
  },
  ...
  addOrganizationMember: async (data: {
    first_name: string;
    last_name: string;
    member_email: string;
    member_id: string;
...
export const dashboardApi = {
  ...
  getMetrics: async (range: string) => {
    return api.get<DashboardMetricsResponse>(
      `/admin/dashboard/metrics?range=${range}`
    );
  },
  ...
  attachFilesToClaim: async (id: number, attachments: FormData) => {
    return api.post<FileClaimResponse>(
...
export const reportsApi = {
  getList: async (params: {
    ...
    const raw = await api.get<unknown>(`/admin/reports${qs ? `?${qs}` : ""}`);
  ...
  ignore: async (reportId: string) => {
    await api.post(`/admin/reports/${encodeURIComponent(reportId)}/ignore`, {});
  },

  reopen: async (reportId: string) => {
    await api.post(`/admin/reports/${encodeURIComponent(reportId)}/reopen`, {});
  },
...
export const categoriesApi = {
  getCategories: async (search?: string) => {
    ...
    return api.get<CategoryResponse>("/admin/categories?status=active", {
      params,
    });
  },
...
export const coverageProductsApi = {
  ...
  getCoverageProducts: async (page = 1, per_page = 100) => {
    return api.get<CoverageProductsResponse>(
      `/coverage-products?page=${page}&per_page=${per_page}`
    );
  },
  getCoverageProductStart: async (categoryId: number | undefined) => {
    return api.post<CoverageProductStartResponse>("/coverage-products/start", {
  ...
  getCoveragePlans: async (categoryId: number, productPrice: string) => {
    return api.get<CoveragePlansResponse>(
      `/coverage-plans?category_id=${categoryId}&product_price=${productPrice}`
    );
  },
  getExtensionOptions: async (id: number) => {
    return api.get<ExtensionOptionsResponse>(
      `/coverage-products/${id}/extension-options`
    );
  },
  createExtensionPaymentIntent: async (
    id: number,
    data: CreateExtensionPaymentIntentRequest
  ) => {
    return api.post<CreateExtensionPaymentIntentResponse>(
      `/coverage-products/${id}/extension/payment-intent`,
      data
    );
  },
  confirmExtensionPayment: async (
    id: number,
    data: ConfirmExtensionPaymentRequest
  ) => {
    return api.post<ConfirmExtensionPaymentResponse>(
      `/coverage-products/${id}/extension/confirm`,
      data
    );
  },
};
